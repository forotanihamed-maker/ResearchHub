...
| Rate limiting لاگین/چت | بله، ولی **درون‌حافظه‌ای** (`lib/rateLimit.ts`) — روی Vercel serverless، این محافظت فقط در محدوده‌ی یک instance گرم تضمین می‌شود، نه سراسری. برای پایلوت کوچک کافی است؛ برای ترافیک واقعی باید با Redis (مثل Upstash) جایگزین شود. |
